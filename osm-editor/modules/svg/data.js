...
export function svgData(projection, context, dispatch) {
    ...
    function drawData(selection) {
        ...
        paths.enter()
            ...
            // Read simplestyle-spec properties (stroke, fill, fill-opacity)
            // directly from the feature — our Martin SQL function already
            // emits these per layer type (water, highway, building, etc).
            // iD has no built-in support for this, so we wire it in here.
            .style('stroke', function(d) {
                var datagroup = this.parentNode.__data__;
                if (datagroup !== 'stroke') return null;
                return (d.properties && d.properties.stroke) || null;
            })
            .style('stroke-width', function(d) {
                var datagroup = this.parentNode.__data__;
                if (datagroup !== 'stroke') return null;
                return (d.properties && d.properties.stroke) ? 2 : null;
            })
            .style('fill', function(d) {
                var datagroup = this.parentNode.__data__;
                if (datagroup !== 'fill') return null;
                return (d.properties && d.properties.fill) || null;
            })
            .style('fill-opacity', function(d) {
                var datagroup = this.parentNode.__data__;
                if (datagroup !== 'fill') return null;
                var op = d.properties && d.properties['fill-opacity'];
                return (op !== undefined && op !== null) ? op : null;
            });
